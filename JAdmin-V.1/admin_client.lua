...
RegisterNetEvent("jadmin:adminresponse")
RegisterNetEvent("jadmin:amiadmin")
RegisterNetEvent("jadmin:fillBanlist")
RegisterNetEvent("jadmin:requestSpectate")
RegisterNetEvent("jadmin:requestCleanup")

RegisterNetEvent("jadmin:SetSetting")
RegisterNetEvent("jadmin:SetLanguage")

RegisterNetEvent("jadmin:TeleportRequest")
RegisterNetEvent("jadmin:SlapPlayer")
RegisterNetEvent("jadmin:FreezePlayer")
RegisterNetEvent("jadmin:CaptureScreenshot")
RegisterNetEvent("jadmin:GetPlayerList")
RegisterNetEvent("jadmin:GetInfinityPlayerList")
RegisterNetEvent("jadmin:fillCachedPlayers")


AddEventHandler('jadmin:adminresponse', function(perms)
	permissions = perms
...
AddEventHandler('jadmin:SetSetting', function(setting,state)
	settings[setting] = state
	if setting == "button" and state ~= "none" then
		if (not RedM and not tonumber(settings.button)) then
			RegisterKeyMapping('jadmin', 'Open jadmin', 'keyboard', settings.button)
		end
	end
end)

AddEventHandler('jadmin:SetLanguage', function(newstrings)
	strings = newstrings
end)


AddEventHandler("jadmin:fillBanlist", function(thebanlist)
	banlist = thebanlist
end)

AddEventHandler("jadmin:fillCachedPlayers", function(thecached)
	if permissions["player.ban.temporary"] or permissions["player.ban.permanent"] then
		cachedplayers = thecached
	end
end)

AddEventHandler("jadmin:GetPlayerList", function(players)
	playerlist = players
end)

AddEventHandler("jadmin:GetInfinityPlayerList", function(players)
	playerlist = players
end)

RegisterNetEvent("jadmin:getServerAces")
AddEventHandler("jadmin:getServerAces", function(aces,principals)
	add_aces = aces
...
RegisterNetEvent("jadmin:SetLanguage")
AddEventHandler("jadmin:SetLanguage", function()
	if permissions["server.permissions.read"] then
		TriggerServerEvent("jadmin:getServerAces")
	end
end)

RegisterNetEvent("jadmin:NewReport")
AddEventHandler("jadmin:NewReport", function(reportData)
	reports[reportData.id] = reportData
end)

RegisterNetEvent("jadmin:RemoveReport")
AddEventHandler("jadmin:RemoveReport", function(reportData)
...
AddEventHandler('jadmin:requestSpectate', function(playerServerId, tgtCoords)
...
AddEventHandler('jadmin:TeleportRequest', function(id, tgtCoords)
	if id then
...
AddEventHandler('jadmin:SlapPlayer', function(slapAmount)
...
RegisterCommand("kick", function(source, args, rawCommand)
	...
	if args[1] and tonumber(args[1]) then
		TriggerServerEvent("jadmin:kickPlayer", tonumber(args[1]), reason)
	end
end, false)
...
RegisterCommand("ban", function(source, args, rawCommand)
	if args[1] and tonumber(args[1]) then
		...
		if args[1] and tonumber(args[1]) then
			TriggerServerEvent("jadmin:banPlayer", tonumber(args[1]), reason, false, GetPlayerName(args[1]))
		end
	end
end, false)

AddEventHandler('jadmin:FreezePlayer', function(toggle)
	frozen = toggle
...
AddEventHandler('jadmin:CaptureScreenshot', function(toggle, url, field)
	exports['screenshot-basic']:requestScreenshotUpload(GetConvar("ja_screenshoturl", 'https://wew.wtf/upload.php'), GetConvar("ja_screenshotfield", 'files[]'), function(data)
		TriggerServerEvent("jadmin:TookScreenshot", data)
	end)
end)
...
RegisterNetEvent("jadmin:showNotification")
AddEventHandler("jadmin:showNotification", function(text, important)
	if not RedM then
