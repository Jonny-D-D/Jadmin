...
Citizen.CreateThread(function()
	while true do 
		...
		for i, player in pairs(CachedPlayers) do 
			if player.droppedTime and (os.time() > player.droppedTime+GetConvarInt("ja_playerCacheExpiryTime", 1800)) then
...
function sendRandomReminder()
	reminderTime = GetConvarInt("ja_chatReminderTime", 0)
	if reminderTime ~= 0 and #ChatReminders > 0 then
		...
		TriggerClientEvent("chat:addMessage", -1, { args = { "jadmin", reminder } })
	end
end
...
Citizen.CreateThread(function()
	while true do 
		...
		if (GetConvarInt("ja_backupFrequency", 72) ~= 0) and (lastBackupTime+(GetConvarInt("ja_backupFrequency", 72)*3600) < os.time()) then
			createBackup()
		end
		Wait(120000)
	end
	...
	if backup then
		...
		if backupJson then
			...
			for i,ban in pairs(backupJson) do
				...
				TriggerEvent("ja_data:addBan", ban)
				Wait(50)
			end
			...
			PrintDebugMessage("^jadmin:^7 Backup Could not be loaded, in most cases this comes from there being a formatting error, please use a JSON Validator on the file and fix the errors!")
		end

	else
		PrintDebugMessage("^jadmin:^7 Backup Name Invalid or missing from Backups.")
	end
end
...
function createBackup()
	...
	if backupInfos then
		...
		if #backupInfos.backups > GetConvarInt("ja_maxBackupCount", 10) then
...
function getAllPlayerIdentifiers(playerId) --Gets all info that could identify a player
	...
	local tokens = {}
	if GetConvar("ja_useTokenIdentifiers", "true") == "true" then
		if not GetNumPlayerTokens or not GetPlayerToken then
			...
			SetConvar("ja_useTokenIdentifiers", "false")
			PrintDebugMessage("Set ja_useTokenIdentifiers to false for this session.", 1)
			return identifiers
		end
...
AddEventHandler("jadmin:amiadmin", function()
...
RegisterServerEvent("jadmin:GetPlayerList")
AddEventHandler("jadmin:GetPlayerList", function()
	...
	if IsPlayerAdmin(source) then
		...
		TriggerClientEvent("jadmin:GetPlayerList", source, l) 
	end
end)


RegisterServerEvent("jadmin:GetInfinityPlayerList")
AddEventHandler("jadmin:GetInfinityPlayerList", function()
	...
	if IsPlayerAdmin(source) then
		...
		TriggerClientEvent("jadmin:GetInfinityPlayerList", source, l) 
	end
end)


RegisterServerEvent("jadmin:requestCachedPlayers")
AddEventHandler('jadmin:requestCachedPlayers', function()
	...
	if (DoesPlayerHavePermission(source, "player.ban.temporary") or DoesPlayerHavePermission(source, "player.ban.permanent")) then
		TriggerClientEvent("jadmin:fillCachedPlayers", src, CachedPlayers)
...
function DoesPlayerHavePermission(player, object)
	...
	if string.find(object, "jadmin.") then -- compatability with outdated plugins
		object = string.gsub(object, "jadmin.", "")
	end
	object = "jadmin."..object
...
RegisterCommand("ja_addShortcut", function(source, args, rawCommand)
...
RegisterCommand("ja_addReminder", function(source, args, rawCommand)
	if args[1] and DoesPlayerHavePermission(source, "server.reminder.add") then
		local text = string.gsub(rawCommand, "ja_addReminder ", "")
...
RegisterCommand("ja_testWebhook", function(source, args, rawCommand)
...
RegisterCommand("ja_excludeWebhookFeature", function(source, args, rawCommand)
    if DoesPlayerHavePermission(source, "server") then
...
RegisterCommand("ja_createBackup", function(source, args, rawCommand)
	if DoesPlayerHavePermission(source, "server") then
		createBackup()
    end
end, false)

RegisterCommand("ja_loadBackup", function(source,args,rawCommand)
...
RegisterCommand("ja_generateSupportFile", function(source, args, rawCommand)
	if DoesPlayerHavePermission(source, "server") then
		PrintDebugMessage("Creating Support File....^7\n", 1)

		local supportData = {}

		PrintDebugMessage("Collecting jadmin Config....^7\n", 1)

		supportData.config = {
			gamename = GetConvar("gamename", "not-rdr3"),
			version = GetVersion(),
			ja_moderationNotification = GetConvar("ja_moderationNotification", "false"),
			ja_screenshoturl = GetConvar("ja_screenshoturl", 'https://wew.wtf/upload.php'),
			onesync = GetConvar("onesync", "off"),
			steam_webApiKey = GetConvar("steam_webApiKey", "none"),
			ja_LanguageName = GetConvar("ja_LanguageName", "en"),
			ja_enableDebugging = GetConvar("ja_enableDebugging", "false"),
			ja_logLevel = GetConvar("ja_logLevel", 1),
			ja_minIdentifierMatches = GetConvarInt("ja_minIdentifierMatches", 2),
			ja_MenuButton = GetConvar("ja_MenuButton", "none"),
			ja_alwaysShowButtons = GetConvar("ja_alwaysShowButtons", "false"),
			ja_enableCallAdminCommand = GetConvar("ja_enableCallAdminCommand", "false"),
			ja_enableReportCommand = GetConvar("ja_enableReportCommand", "false"),
			ja_defaultMinReports = GetConvarInt("ja_defaultMinReports", 3),
			ja_MinReportPlayers = GetConvarInt("ja_MinReportPlayers", 12),
			ja_MinReportModifierEnabled = GetConvar("ja_MinReportModifierEnabled", "true"),
			ja_ReportBanTime = GetConvarInt("ja_ReportBanTime", 86400),
			ja_custombanlist = GetConvar("ja_custombanlist", "false"),
			ja_maxWarnings = GetConvarInt("ja_maxWarnings", 3),
			ja_warnAction = GetConvar("ja_warnAction", "kick"),
			ja_warningBanTime = GetConvarInt("ja_warningBanTime", 604800),
			ja_enableSplash = GetConvar("ja_enableSplash", "true"),
			ja_playerCacheExpiryTime = GetConvarInt("ja_playerCacheExpiryTime", 900),
			ja_chatReminderTime = GetConvarInt("ja_chatReminderTime", 0),
			ja_backupFrequency = GetConvarInt("ja_backupFrequency", 72),
			ja_maxBackupCount = GetConvarInt("ja_maxBackupCount", 10),
			ja_useTokenIdentifiers = GetConvar("ja_useTokenIdentifiers", "true"),
			ja_enableTelemetry = GetConvar("ja_enableTelemetry", "true"),
		}
...
Citizen.CreateThread(function()
	...
	if resourcemeta then
		...
		PrintDebugMessage("Found __resource.lua file in jadmin Folder and attempted deletion.", 1)
	end

	ExcludedWebhookFeatures = {}
	AnonymousAdmins = {}

	local strfile = LoadResourceFile(GetCurrentResourceName(), "language/"..GetConvar("ja_LanguageName", "en")..".json")
	if strfile then
		strings = json.decode(strfile)[1]
	else
		strings = {language=GetConvar("ja_LanguageName", "en")}
	end
	
	
	moderationNotification = GetConvar("ja_moderationNotification", "false")
	reportNotification = GetConvar("ja_reportNotification", "false")
	detailNotification = GetConvar("ja_detailNotification", "false")
	minimumMatchingIdentifierCount = GetConvarInt("ja_minIdentifierMatches", 2)
	
	RegisterServerEvent('jadmin:amiadmin')
	AddEventHandler('jadmin:amiadmin', function()
		...
		TriggerClientEvent("jadmin:adminresponse", source, perms)
		...
		local key = GetConvar("ja_MenuButton", "none")
		if RedM then
			key = GetConvar("ja_MenuButton", "PhotoModePc")
		end
		TriggerClientEvent("jadmin:SetSetting", source, "button", key)
		if GetConvar("ja_alwaysShowButtons", "false") == "true" then
			TriggerClientEvent("jadmin:SetSetting", source, "forceShowGUIButtons", true)
		else
			TriggerClientEvent("jadmin:SetSetting", source, "forceShowGUIButtons", false)
		end
		if updateAvailable then
			TriggerClientEvent("jadmin:SetSetting", source, "updateAvailable", updateAvailable)
		end
		...
		if os.date("%d/%m") == "22/08" then
			...
			TriggerClientEvent("jadmin:SetSetting", source, "alternativeTitle", "~b~Today is jadmin's "..age..""..ordinal.." birthday! :)")
		elseif os.date("%m") == "06" and (tonumber(os.date("%d")) >= 1 and tonumber(os.date("%d")) <= 7)  then
			TriggerClientEvent("jadmin:SetSetting", source, "alternativeLogo", "pride")
		end


		if (infinity) then 
			TriggerClientEvent("jadmin:SetSetting", source, "infinity", true)
		end
		
		TriggerClientEvent("jadmin:SetLanguage", source, strings)
		
	end)
	
	RegisterServerEvent("jadmin:kickPlayer")
	AddEventHandler('jadmin:kickPlayer', function(playerId,reason)
	...
	RegisterServerEvent("jadmin:requestSpectate")
	AddEventHandler('jadmin:requestSpectate', function(playerId)
		if DoesPlayerHavePermission(source, "player.spectate") then
			...
			TriggerClientEvent("jadmin:requestSpectate", source, playerId, tgtCoords)
	...
	RegisterServerEvent("jadmin:requestCleanup")
	AddEventHandler('jadmin:requestCleanup', function(type)
		if DoesPlayerHavePermission(source, "server.cleanup."..type) then
			...
			TriggerClientEvent("jadmin:requestCleanup", source, type)
			local preferredWebhook = detailNotification ~= "false" and detailNotification or moderationNotification
	...
	RegisterServerEvent("jadmin:SetGameType")
	AddEventHandler('jadmin:SetGameType', function(text)
	...
	RegisterServerEvent("jadmin:SetMapName")
	AddEventHandler('jadmin:SetMapName', function(text)
		if DoesPlayerHavePermission(source, "server.convars") then
	...
	RegisterServerEvent("jadmin:StartResource")
	AddEventHandler('jadmin:StartResource', function(text)
	...
	RegisterServerEvent("jadmin:StopResource")
	AddEventHandler('jadmin:StopResource', function(text)
	...
	RegisterServerEvent("jadmin:SetConvar")
	AddEventHandler('jadmin:SetConvar', function(convarname, convarvalue)
		if DoesPlayerHavePermission(source, "server.convars") then
	...
	RegisterServerEvent("jadmin:banPlayer")
	AddEventHandler('jadmin:banPlayer', function(playerId,reason,expires)
	...
	RegisterServerEvent("jadmin:offlinebanPlayer")
	AddEventHandler('jadmin:offlinebanPlayer', function(playerId,reason,expires)
	...
	AddEventHandler('banCheater', function(playerId,reason)
		Citizen.Trace("^1jadmin^7: the banCheater event is ^1deprecated^7 and has been removed! Please adjust your ^3"..GetInvokingResource().."^7 Resource to use jadmin:addBan instead.")
	end)
	
	AddEventHandler("jadmin:addBan", function(playerId,reason,expires, offline)
		if offline then 
	...
	RegisterServerEvent("jadmin:updateBanlist")
	AddEventHandler('jadmin:updateBanlist', function(playerId)
		local src = source
		if DoesPlayerHavePermission(source, "player.kick") then
			updateBlacklist(false,true)
			Citizen.Wait(300)
			TriggerClientEvent("jadmin:fillBanlist", src, blacklist)
			PrintDebugMessage("Banlist Refreshed by "..getName(src,true), 3)
		end
	end)
	
	RegisterServerEvent("jadmin:requestBanlist")
	AddEventHandler('jadmin:requestBanlist', function()
		local src = source
		if DoesPlayerHavePermission(source, "player.kick") then
			TriggerClientEvent("jadmin:fillBanlist", src, blacklist)
	...
	RegisterCommand("spectate", function(source, args, rawCommand)
		...
		if args[1] and tonumber(args[1]) and DoesPlayerHavePermission(source, "player.spectate") then
			if getName(args[1]) then
				TriggerClientEvent("jadmin:requestSpectate", source, args[1])
			else
				TriggerClientEvent("chat:addMessage", source, { args = { "jadmin", GetLocalisedText("playernotfound") } })
			end
		end
	end, false)
	...
	RegisterCommand("unban", function(source, args, rawCommand)
		if args[1] and DoesPlayerHavePermission(source, "player.unban") then
			...
			if (source ~= 0) then
				TriggerClientEvent("chat:addMessage", source, { args = { "jadmin", GetLocalisedText("done") } })
			else
	...
	RegisterCommand("slap", function(source, args, rawCommand)
		if args[1] and args[2] and DoesPlayerHavePermission(source, "player.slap") then
			...
			TriggerClientEvent("jadmin:SlapPlayer", args[1], args[2])
		end
	end, false)	


	--- Commands for Normal Users
	RegisterCommand(GetConvar("ja_callAdminCommandName", "calladmin"), function(source, args, rawCommand)
		if GetConvar("ja_enableCallAdminCommand", "false") == "true" then
			local time = os.time()
			local cooldowntime = GetConvarInt("ja_callAdminCooldown", 60)
			local source=source
			if cooldowns[source] and cooldowns[source] > (time - cooldowntime) then
				TriggerClientEvent('chat:addMessage', source, { 
					...
					args = { "^3!!jadmin!!^7", "You must wait before using this again!" }, color = { 255, 255, 255 } 
				})
				return
			end
			...
			--TriggerClientEvent('chatMessage', source, "^3jadmin^7", {255,255,255}, GetLocalisedText("admincalled"))
			TriggerClientEvent('chat:addMessage', source, { 
				template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(253, 53, 53, 0.6); border-radius: 3px;"><i class="fas fa-crown"></i> {0}: {1}</div>',
				args = { "^3!!jadmin!!^7", GetLocalisedText("admincalled") }, color = { 255, 255, 255 } 
			})
	...
	PlayerReports = {}
	RegisterCommand(GetConvar("ja_reportCommandName", "report"), function(source, args, rawCommand)
		if GetConvar("ja_enableReportCommand", "false") == "true" then
			local source = source
			local id = args[1]
			local valid = false
			local minimumreports = GetConvarInt("ja_defaultMinReports", 3)
			if GetConvar("ja_MinReportModifierEnabled", "true") == "true" then
				if #GetPlayers() > GetConvarInt("ja_MinReportPlayers", 12) then
					minimumreports = math.round(#GetPlayers()/GetConvarInt("ja_MinReportModifier", 4),0)
				end
			end
			...
			if id and valid then
				...
				if addReport then
					...
					if GetConvar("ja_enableReportScreenshots", "true") == "true" then
						TriggerEvent("jadmin:TakeScreenshot", id)
					end
					...
					TriggerClientEvent('chat:addMessage', source, { 
						...
						args = { "^3jadmin^7", GetLocalisedText("successfullyreported") }, color = { 255, 255, 255 } 
					})
					if #PlayerReports[id] >= minimumreports then
						TriggerEvent("EasyjadminAdmin:addBan", id, string.format(GetLocalisedText("reportbantext"), minimumreports), os.time()+GetConvarInt("ja_ReportBanTime", 86400))
					end
				else
					TriggerClientEvent('chat:addMessage', source, { 
						template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(253, 53, 53, 0.6); border-radius: 5px;"><i class="fas fa-user-crown"></i> {0}:<br> {1}</div>',
						args = { "^3jadmin^7", GetLocalisedText("alreadyreported") }, color = { 255, 255, 255 } 
					})
				end
			else
				TriggerClientEvent('chat:addMessage', source, { 
					template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(253, 53, 53, 0.6); border-radius: 5px;"><i class="fas fa-user-crown"></i> {0}:<br> {1}</div>',
					args = { "^3jadmin^7", GetLocalisedText("reportedusageerror") }, color = { 255, 255, 255 } 
				})
			end
		end
	end, false)
	
	RegisterServerEvent("jadmin:TeleportPlayerToCoords")
	AddEventHandler('jadmin:TeleportPlayerToCoords', function(playerId,tgtCoords)
		if DoesPlayerHavePermission(source, "player.teleport.single") then
			...
			TriggerClientEvent("jadmin:TeleportRequest", playerId, false, tgtCoords)
		end
	end)

	RegisterServerEvent("jadmin:TeleportAdminToPlayer")
	AddEventHandler("jadmin:TeleportAdminToPlayer", function(id)
		if not CachedPlayers[id].dropped and DoesPlayerHavePermission(source, "player.teleport.single") then
			...
			TriggerClientEvent('jadmin:TeleportRequest', source, id,tgtCoords)
		else
			print('JADMIN FAILED TO TELEPORT'..source..' TO ID: '..id)
		end
	end)
	
	RegisterServerEvent("jadmin:SlapPlayer")
	AddEventHandler('jadmin:SlapPlayer', function(playerId,slapAmount)
		if DoesPlayerHavePermission(source, "player.slap") then
			...
			TriggerClientEvent("jadmin:SlapPlayer", playerId, slapAmount)
		end
	end)
	
	RegisterServerEvent("jadmin:FreezePlayer")
	AddEventHandler('jadmin:FreezePlayer', function(playerId,toggle)
		if DoesPlayerHavePermission(source, "player.freeze") then
			local preferredWebhook = detailNotification ~= "false" and detailNotification or moderationNotification
			if toggle then
			...
			TriggerClientEvent("jadmin:FreezePlayer", playerId, toggle)
		end
	end)
	
	RegisterServerEvent("jadmin:TookScreenshot")
	
	RegisterServerEvent("jadmin:TakeScreenshot")
	AddEventHandler('jadmin:TakeScreenshot', function(playerId)
		if scrinprogress then
			TriggerClientEvent("chat:addMessage", source, { args = { "jadmin", GetLocalisedText("screenshotinprogress") } })
			return
		end
		local src=source
		...
		if DoesPlayerHavePermission(source, "player.screenshot") then
			scrinprogress = true
			thistemporaryevent = AddEventHandler("jadmin:TookScreenshot", function(result)
				...
				TriggerClientEvent("chat:addMessage", src, { args = { "jadmin", string.format(GetLocalisedText("screenshotlink"), res) } })
			...
			TriggerClientEvent("jadmin:CaptureScreenshot", playerId)
			local timeoutwait = 0
			repeat
				...
				if timeoutwait == 5 then
					...
					TriggerClientEvent("chat:addMessage", src, { args = { "jadmin", "Screenshot Failed!" } })
				end
			until not scrinprogress
		end
	end)
	
	RegisterServerEvent("jadmin:unbanPlayer")
	AddEventHandler('jadmin:unbanPlayer', function(banId)
		local thisBan = nil
	...
	RegisterServerEvent("jadmin:mutePlayer")
	AddEventHandler('jadmin:mutePlayer', function(playerId)
		local src = source
		if DoesPlayerHavePermission(src,"player.mute") then
			if not MutedPlayers[playerId] then 
				MutedPlayers[playerId] = true
				TriggerClientEvent("chat:addMessage", src, { args = { "jadmin", getName(playerId) .. " " .. GetLocalisedText("playermuted") } })
				...
				TriggerClientEvent("chat:addMessage", src, { args = { "jadmin", getName(playerId) .. " " .. GetLocalisedText("playerunmuted") } })
	...
	RegisterServerEvent("jadmin:SetAnonymous")
	AddEventHandler('jadmin:SetAnonymous', function(playerId)
	...
	function addNewReport(type, reporter, reported, reason)
		...
		reports[t.id] = t
		for i,_ in pairs(OnlineAdmins) do 
			TriggerClientEvent("jadmin:NewReport", i, t)
		end
		return t.id
	end
		...
		for i, report in pairs(reports) do
			if (index and i == index) then
				for admin,_ in pairs(OnlineAdmins) do 
					TriggerClientEvent("jadmin:RemoveReport", admin, report)
				end
				reports[i] = nil
	...
	function removeSimilarReports(report)
		for i, r in pairs(reports) do
			if (report.reporter and report.reported) and (report.reporter == r.reporter and report.reported == r.reported) then
				for admin,_ in pairs(OnlineAdmins) do 
					TriggerClientEvent("jadmin:RemoveReport", admin, r)
				end
				reports[i] = nil
			end
	...
	RegisterServerEvent("jadmin:RemoveReport")
	AddEventHandler("jadmin:RemoveReport", function(report)
	...
	RegisterServerEvent("jadmin:RemoveSimilarReports")
	AddEventHandler("jadmin:RemoveSimilarReports", function(report)
		if DoesPlayerHavePermission(source, "player.reports.process") then
			SendWebhookMessage(moderationNotification,string.format(GetLocalisedText("adminclosedreport"), getName(source, false, true), report.id), "reports", 16777214)
	...
	function FindInfosinFile(filename)
		...
		elseif filename == "jadmin_permissions.cfg" then
			needsExec = false
		else
		...
		if file then
			...
			for i, line in pairs(lines) do 
				if filename == "server.cfg" then
					needsResourcePerms = false
					if string.find(line, "exec jadmin_permissions.cfg") then
						needsExec = false
					end
				elseif filename == "jadmin_permissions.cfg" then
					needsExec = false
			...
			if needsExec or needsResourcePerms then
				local newLines = {}
				if needsExec then
					table.insert(newLines, "exec jadmin_permissions.cfg")
					table.insert(execs, "jadmin_permissions.cfg")
					PrintDebugMessage("Did not find `exec jadmin_permissions.cfg`, added it automatically", 4)
					changes=true
				end
			...
			if filename == "jadmin_permissions.cfg" then
	...
	Citizen.CreateThread(function()
		...
		function RemoveFromFile(filename, args)
			...
			if file then
				...
				while line do
					if line == args or (filename == "jadmin_permissions.cfg" and line == "") then -- skip lines we dont want, incl. empty lines
					else
						table.insert(lines, line)
					end
					line = file:read("*line")
	...
	RegisterServerEvent("jadmin:getServerAces")
	AddEventHandler("jadmin:getServerAces", function()
		if DoesPlayerHavePermission(source, "server.permissions.read") then
			TriggerClientEvent("jadmin:getServerAces", source, add_aces, add_principals)
		end
	end)
	
	RegisterServerEvent("jadmin:setServerAces")
	AddEventHandler("jadmin:setServerAces", function(aces,principals)
		if DoesPlayerHavePermission(source, "server.permissions.write") then
			...
			for i, ace in pairs(add_aces) do
				...
				if not aces[i] then
					if not ace.file then ace.file = "jadmin_permissions.cfg" end
				...
				elseif aces[i][1] ~= ace[1] or aces[i][2] ~= ace[2] or aces[i][3] ~= ace[3] then
					if not ace.file then ace.file = "jadmin_permissions.cfg" end
					if not aces[i].file then aces[i].file = "jadmin_permissions.cfg" end
					ExecuteCommand("remove_ace "..ace[1].." "..ace[2].." "..ace[3])
					RemoveFromFile(ace.file, ace.oldline or "add_ace "..ace[1].." "..ace[2].." "..ace[3])
			...
			for i, principal in pairs(add_principals) do
				...
				if not principals[i] then
					if not principal.file then principal.file = "jadmin_permissions.cfg" end
				...
				elseif principals[i][1] ~= principal[1] or principals[i][2] ~= principal[2] then
					if not principal.file then principal.file = "jadmin_permissions.cfg" end
					if not principals[i].file then principals[i].file = "jadmin_permissions.cfg" end
	...
	RegisterCommand("convertbanlist", function(source, args, rawCommand)
		if GetConvar("ja_custombanlist", "false") == "true" then
			...
			for i,theBan in ipairs(ob) do
				TriggerEvent("ja_data:addBan", theBan)
				print("processed ban: "..i.."\n")
			end
			content=nil
		else
			print("Custom Banlist is not enabled, converting back to json.")
			TriggerEvent('ja_data:retrieveBanlist', function(banlist)
				blacklist = banlist
	...
	function getName(src,anonymousdisabled,identifierenabled)
		local identifierPref = GetConvar("ja_logIdentifier", "steam")
	...
	function updateBan(id,newData)
		if id and newData and newData.identifiers and newData.banid and newData.reason and newData.expire then 
			...
			SaveResourceFile(GetCurrentResourceName(), "banlist.json", json.encode(blacklist, {indent = true}), -1)
			if GetConvar("ja_custombanlist", "false") == "true" then 
				TriggerEvent("ja_data:updateBan", newData)
			end
		end
	end
	...
	function updateBlacklist(data,remove, forceChange)
		...
		if GetConvar("ja_custombanlist", "false") == "true" then 
			PrintDebugMessage("You are using a Custom Banlist System, this is ^3not currently supported^7 and WILL cause issues! Only use this if you know what you are doing, otherwise, disable ja_custombanlist.", 1)
			if data and not remove then
				addBan(data)
				TriggerEvent("ja_data:addBan", data)
				...
				TriggerEvent('ja_data:retrieveBanlist', function(banlist)
					...
					for i,theBan in ipairs(blacklist) do
						if theBan.expire < os.time() then
							...
							TriggerEvent("ja_data:removeBan", theBan)
						end
					end
				end)
			end
			...
			for i,ban in ipairs(blacklist) do
				for index,id in ipairs(ban.identifiers) do
					if identifier == id then
						...
						SaveResourceFile(GetCurrentResourceName(), "banlist.json", json.encode(blacklist, {indent = true}), -1)
						if GetConvar("ja_custombanlist", "false") == "true" then 
							TriggerEvent("ja_data:removeBan", ban)
						end
	...
	RegisterServerEvent("jadmin:warnPlayer")
	AddEventHandler('jadmin:warnPlayer', function(id, reason)
		local src = source
		if DoesPlayerHavePermission(src,"player.warn") and not DoesPlayerHavePermission(id,"immune") then
			reason = formatShortcuts(reason)
			local maxWarnings = GetConvarInt("ja_maxWarnings", 3)
			...
			if WarnedPlayers[id].warns >= maxWarnings then
				if GetConvar("ja_warnAction", "kick") == "kick" then
				...
				elseif GetConvar("ja_warnAction", "kick") == "ban" then
					...
					local expires = os.time()+GetConvarInt("ja_warningBanTime", 604800)
	...
	AddEventHandler("jadmin:GetVersion", function(cb)
		cb(GetVersion())
	end)
	...
	AddEventHandler('playerConnecting', function(playerName, setKickReason)
		...
		if not blacklist then
			...
			print("jadmin: ^1Failed^7 to load Banlist!\n")
			print("jadmin: Please check this error soon, ^1Bans *will not* work!^7\n")
			print("^1-^2-^3-^4-^5-^6-^8-^9-^1-^2-^3-^4-^5-^6-^8-^9-^1-^2-^3-^3!^1FATAL ERROR^3!^3-^2-^1-^9-^8-^6-^5-^4-^3-^2-^1-^9-^8-^6-^5-^4-^3-^2-^7\n")
			setKickReason("\n\njadmin: A fatal error occured, please contact a Server Administrator to resolve this issue.")
			CancelEvent()
		 	return
		end
	...
	AddEventHandler('chatMessage', function(Source, Name, Msg)
		if MutedPlayers[Source] then
			CancelEvent()
			TriggerClientEvent("chat:addMessage", Source, { args = { "jadmin", GetLocalisedText("playermute") } })
		end
	end)
		...
		local embed = {
			{
				...
				["title"] = "**"..(title or "jadmin").."**",
				["description"] = message,
				["footer"] = {
					["text"] = "jadmin on "..formatDateString(os.time()),
				},
			}
		}
		if image then
	...
	local updatePath = "/Blumlaut/jadmin"
	local resourceName = "jadmin ("..GetCurrentResourceName()..")"
	function checkVersion(err,response, headers)
		if err == 200 then
			...
			PrintDebugMessage("jadmin Version Check failed, please make sure its updated!")
		end
		...
		if tonumber(GetConvar("ja_MenuButton", "none")) then -- let people know they broke stuff
			PrintDebugMessage("ja_MenuButton has not been updated, please follow the updating instructions here:\nhttps://discord.gg/F8E8pm4sKQ", 1)
			PrintDebugMessage("If you do not correct this, your Menu key will cease working in the near future.", 1)
		elseif GetConvar("ja_MenuButton", "none") == "none" then
			PrintDebugMessage("ja_MenuButton is not defined, jadmin can only be opened using the /jadmin command, to define a key:\nhttps://discord.gg/F8E8pm4sKQ", 1)
		end
	...
	function loopTelemetryUpdate()
		if GetConvar("ja_enableTelemetry", "true") == "false" then
	...
	if GetConvar("ja_enableTelemetry", "true") == "true" then
		loopTelemetryUpdate()
	end
	if GetConvar("ja_enableSplash", "true") == "true" then
