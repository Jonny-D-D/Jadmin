...
function PrintDebugMessage(msg,level)
	loglevel = (GetConvarInt("ja_logLevel", 1))
...
if IsDuplicityVersion() then
	if GetConvar("ja_enableDebugging", "false") ~= "false" or GetConvarInt("ja_logLevel", 1) ~= 1 then
		SetConvar("ja_enableDebugging", "false")
		if GetConvarInt("ja_logLevel", 1) == 1 then
			SetConvar("ja_logLevel", 3)
		end
		if GetConvarInt("ja_logLevel", 1) > 1 then
			PrintDebugMessage("Debug Messages Enabled, Verbosity is ^2"..GetConvarInt("ja_logLevel", 1).."^7.", 2)
		end
	else
...
function formatDateString(string)
	local dateFormat = GetConvar("ja_dateFormat", '%d/%m/%Y 	%H:%M:%S')
