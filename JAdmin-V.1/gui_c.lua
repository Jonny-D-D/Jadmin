...
RegisterCommand('jadmin', function(source, args)
	CreateThread(function()
		if not isAdmin == true then
			TriggerServerEvent("jadmin:amiadmin")
			local waitTime = 0
		...
		if not RedM and isAdmin then
			playerlist = nil
			TriggerServerEvent("jadmin:GetInfinityPlayerList") -- shitty fix for bigmode
			repeat
				Wait(10)
			until playerlist
		end
		...
		if strings and isAdmin then
			...
			if mainMenu and mainMenu:Visible() then
				...
				_menuPool:Remove()
				TriggerEvent("jadmin:MenuRemoved")
				collectgarbage()
			else
				GenerateMenu()
...
Citizen.CreateThread(function()
	...
	TriggerServerEvent("jadmin:amiadmin")
	TriggerServerEvent("jadmin:requestBanlist")
	TriggerServerEvent("jadmin:requestCachedPlayers")
...
function GenerateMenu() -- this is a big ass function
	...
	mainMenu = NativeUI.CreateMenu("", subtitle, menuOrientation, 0, "jadmin", "banner-gradient", "logo")
	...
	TriggerEvent("jadmin:BuildMainMenuOptions")
	...
	userSearch.Activated = function(ParentMenu, SelectedItem)
		...
		if result and result ~= "" then
			...
			if found and (#temp > 1) then
				...
				local resultMenu = NativeUI.CreateMenu("Search Results", searchsubtitle, menuOrientation, 0, "jadmin", "banner-gradient", "logo")
	...
	for i,thePlayer in pairs(players) do
		...
		if permissions["player.kick"] then
			...
			thisItem.Activated = function(ParentMenu,SelectedItem)
				...
				TriggerServerEvent("jadmin:kickPlayer", thePlayer.id, KickReason)
				BanTime = 1
				BanReason = ""
				_menuPool:CloseAllMenus()
				Citizen.Wait(800)
		...
		if permissions["player.ban.temporary"] or permissions["player.ban.permanent"] then
			...
			thisItem.Activated = function(ParentMenu,SelectedItem)
				...
				TriggerServerEvent("jadmin:banPlayer", thePlayer.id, BanReason, banLength[BanTime].time, thePlayer.name )
				BanTime = 1
				BanReason = ""
				_menuPool:CloseAllMenus()
				Citizen.Wait(800)
		...
		if permissions["player.mute"] then			
			...
			thisItem.Activated = function(ParentMenu,SelectedItem)
				TriggerServerEvent("jadmin:mutePlayer", thePlayer.id)
			end
		end
		...
		if permissions["player.spectate"] then
			...
			thisPlayer:AddItem(thisItem)
			thisItem.Activated = function(ParentMenu,SelectedItem)
				TriggerServerEvent("jadmin:requestSpectate",thePlayer.id)
			end
		end
				...
				if item == thisItem then
					...
					if i == GetLocalisedText("teleporttoplayer") then
						if settings.infinity and not RedM then
							TriggerServerEvent('jadmin:TeleportAdminToPlayer', thePlayer.id)
						else
						...
						TriggerServerEvent("jadmin:TeleportPlayerToCoords", thePlayer.id, coords)
					end
				end
			end
		end
		...
		if permissions["player.slap"] then
			...
			thisItem.OnSliderSelected = function(index)
				TriggerServerEvent("jadmin:SlapPlayer", thePlayer.id, index*10)
			end
		end
			...
			thisItem.OnListSelected = function(sender, item, index)
					if item == thisItem then
							...
							if i == GetLocalisedText("on") then
								TriggerServerEvent("jadmin:FreezePlayer", thePlayer.id, true)
							else
								TriggerServerEvent("jadmin:FreezePlayer", thePlayer.id, false)
							end
					end
			end
		end
		...
		if permissions["player.screenshot"] then
			...
			thisPlayer:AddItem(thisItem)
			thisItem.Activated = function(ParentMenu,SelectedItem)
				TriggerServerEvent("jadmin:TakeScreenshot", thePlayer.id)
			end
		end
		...
		if permissions["player.warn"] then
			...
			thisItem.Activated = function(ParentMenu,SelectedItem)
				...
				TriggerServerEvent("jadmin:warnPlayer", thePlayer.id, WarnReason)
				BanTime = 1
				BanReason = ""
				_menuPool:CloseAllMenus()
				Citizen.Wait(800)
		...
		TriggerEvent("jadmin:BuildPlayerOptions", thePlayer.id)
	...
	if permissions["player.reports.view"] then
		...
		for i, report in pairs(reports) do
			...
			if permissions["player.reports.process"] then
				...
				thisMenu:AddItem(thisItem)
				thisItem.Activated = function(ParentMenu,SelectedItem)
					TriggerServerEvent("jadmin:RemoveReport", report)
					_menuPool:CloseAllMenus()
					Citizen.Wait(800)
					GenerateMenu()
					reportViewer:Visible(true)
				...
				thisMenu:AddItem(thisItem)
				thisItem.Activated = function(ParentMenu,SelectedItem)
					TriggerServerEvent("jadmin:RemoveSimilarReports", report)
					_menuPool:CloseAllMenus()
					Citizen.Wait(800)
					GenerateMenu()
					reportViewer:Visible(true)
	...
	if permissions["player.teleport.everyone"] then
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			TriggerServerEvent("jadmin:TeleportPlayerToCoords", -1, pCoords)
		end
	end
		...
		for i, cachedplayer in pairs(cachedplayers) do
			if cachedplayer.droppedTime and not cachedplayer.immune then
				...
				thisItem.Activated = function(ParentMenu,SelectedItem)
					...
					TriggerServerEvent("jadmin:offlinebanPlayer", cachedplayer.id, BanReason, banLength[BanTime].time, cachedplayer.name)
					BanTime = 1
					BanReason = ""
					_menuPool:CloseAllMenus()
					Citizen.Wait(800)
				...
				TriggerEvent("jadmin:BuildCachedOptions", cachedplayer.id)
			end
		end
	end
	...
	if permissions["server.convars"] then
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			if result then
				TriggerServerEvent("jadmin:SetGameType", result)
			end
		end
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			if result then
				TriggerServerEvent("jadmin:SetMapName", result)
			end
		end
	end
	...
	if permissions["server.resources.start"] then
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			if result then
				TriggerServerEvent("jadmin:StartResource", result)
			end
		end
	end
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			if result then
				if result ~= GetCurrentResourceName() and result ~= "NativeUI" then
					TriggerServerEvent("jadmin:StopResource", result)
				else
					TriggerEvent("chat:addMessage", { args = { "jadmin", GetLocalisedText("badidea") } })
				end
			end
		end
	end
	...
	if permissions["server.convars"] then
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			if result then
				...
				if result2 then
					TriggerServerEvent("jadmin:SetConvar", result, result2)
				end
			end
		end
	end
	...
	if permissions["player.unban"] then
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			if foundBan then
				...
				mainMenu = NativeUI.CreateMenu("", "~b~Ban Infos", menuOrientation, 0, "jadmin", "banner-gradient", "logo")
				_menuPool:Add(mainMenu)
				...
				mainMenu:AddItem(thisItem)
				thisItem.Activated = function(ParentMenu,SelectedItem)
					TriggerServerEvent("jadmin:unbanPlayer", banlist[foundBanid].banid)
					TriggerServerEvent("jadmin:requestBanlist")
					_menuPool:CloseAllMenus()
					Citizen.Wait(800)
					GenerateMenu()
			...
			if i<(banlistPage*10)+1 and i>(banlistPage*10)-10 then
				if theBanned then
					...
					thisItem.Activated = function(ParentMenu,SelectedItem)
						...
						mainMenu:AddItem(thisItem)
						thisItem.Activated = function(ParentMenu,SelectedItem)
							TriggerServerEvent("jadmin:unbanPlayer", banlist[i].banid)
							TriggerServerEvent("jadmin:requestBanlist")
							_menuPool:CloseAllMenus()
							Citizen.Wait(800)
							GenerateMenu()
		...
		thisItem.OnListSelected = function(sender, item, index)
			if item == thisItem then
					...
					if i == GetLocalisedText('cars') then
						TriggerServerEvent("jadmin:requestCleanup", "cars")
					elseif i == GetLocalisedText('peds') then
						TriggerServerEvent("jadmin:requestCleanup", "peds")
					else
						TriggerServerEvent("jadmin:requestCleanup", "props")
					end
			end
		end
	end
	...
	if permissions["server.permissions.read"] then
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			TriggerServerEvent("jadmin:getServerAces")
			_menuPool:CloseAllMenus()
			Citizen.Wait(800)
			GenerateMenu()
	...
	if permissions["server.permissions.read"] and permissions["server.permissions.write"] then
		...
		permissionEditor:AddItem(thisItem)
		thisItem.Activated = function(ParentMenu,SelectedItem)
			TriggerServerEvent("jadmin:setServerAces",add_aces, add_principals)
			_menuPool:CloseAllMenus()
			Citizen.Wait(800)
			GenerateMenu()
			permissionEditor:Visible(true)
	...
	TriggerEvent("jadmin:BuildServerManagementOptions")
	...
	if permissions["player.unban"] then
		...
		settingsMenu:AddItem(thisItem)
		thisItem.Activated = function(ParentMenu,SelectedItem)
			TriggerServerEvent("jadmin:updateBanlist")
		end
	end
	...
	if permissions["anon"] then
		...
		settingsMenu.OnCheckboxChange = function(sender, item, checked_)
			if item == thisItem then
				anonymous = checked_
				TriggerServerEvent("jadmin:SetAnonymous", checked_)
			end
		end
	end
	...
	TriggerEvent("jadmin:BuildSettingsOptions")
	_menuPool:ControlDisablingEnabled(false)
	_menuPool:MouseControlsEnabled(false)
	...
	while true do
		...
		if drawInfo then
			...
			if (not RedM and IsControlJustPressed(0,103) or (RedM and IsControlJustReleased(0, Controls["Enter"]))) then
				...
				TriggerEvent('jadmin:FreezePlayer', false)
